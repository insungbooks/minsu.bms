		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-7">
					...
					<hr>
					<div>
						<em><%= searchResult.getWriter() %> </em><span class="bar">|</span><em><%= searchResult.getCompany() %> </em><span
							class="bar">|</span><%= searchResult.getPubliDate() %>
					</div>
					<hr>
					<p>
						<em>정가</em> <%= searchResult.getBookPrice() %>원
					</p>
					<p>
						<em>판매가</em> <%= searchResult.getBookPrice()*9/10 %>원
					</p>
					<br>
					<hr>
					<p>배송료 : 2500원</p>
		...
		<div class="container">
			...
			<nav class="productInfo">
				<a href="../../guest/basket/Basket.jsp"><button type="submit"
						class="btn btn-default">장바구니넣기</button></a> <a
					href="../../guest/purchase/payment.jsp"><button type="button"
						class="btn btn-default">바로구매하기</button></a>
			</nav>
			<br>
			<hr>
			...
			<div class="container">
				...
				<div class="tab-content">
					<div id="productInfo" class="tab-pane fade in active">
						<h3>책소개</h3>
						<div class="text" style="border: 1px solid black;">대통령 말이면 안
							되는 일이 없는 이 나라에서 대통령이 올바른 판단력을 지니고 있지 못하다면 그 뒷감당은 누가 해야 하는가. 결국
							우리는 지금 비싼 대가를 치르고 있다. 이 상처를 치유하는 데 오랜 세월이 필요할 것이다. 저자는 이제 유권자들도
							정신 차려야 한다고 일갈한다.</div>
						<hr>
						<h3>저자소개</h3>
						<div class="text" style="border: 1px solid black;">
							<em>오홍근</em>1950년생으로 경희대 의대를 졸업하고 동 대학원에서 박사학위를 받았으며, 캐나다
							클락정신의학연구소 수석연구원, 캐나다 토론토의대 및 캐나다 자연치료의과대학 교수를 역임했다. 한국 최초의 자연치료의학
							박사이기도 한 그는 꾸준한 연구와 임상 실험 등을 통해 국내는 물론 전 세계에 향기요법을 과학화, 대중화하는 데
							앞장서고 있다. 2007년 자연의학과 보완대체의학 분야의 국제적 학술 활동, 정신의학과 심신의학 분야, 스트레스 관련
							질환에 대한 연구 업적을 인정받아 세계인명사전 후즈 후Marquis Who's Who in the World 2007
							에 등재된 바 있다.
						</div>
						<h3>목차</h3>
						<div class="text" style="border: 1px solid black;">
							...
							<p>반공권 독점·악용, 이젠 내려놓을 때</p>
						</div>
						<hr>
						<h3>출판사 서평</h3>
						<div class="text" style="border: 1px solid black;">『민주주의의
							원칙에 따라 대통령을 뽑았더니 박근혜 정부 꼴이라면 민주주의를 폐기해야 마땅하지 않을까. 대통령 말이면 안 되는 일이
							없는 이 나라에서 대통령이 올바른 판단력을 지니고 있지 못하다면 그 뒷감당은 누가 해야 하는가. 결국 우리는 지금
							비싼 대가를 치르고 있다. 이 상처를 치유하는 데 오랜 세월이 필요할 것이다. 저자는 이제 유권자들도 정신 차려야
							한다고 일갈한다. 주인 노릇을 제대로 하라는 말이다. 그러나 희망의 끈을 놓지 않을 수 있는 이유는 지금의 이 과정이
							결국 새로운 기회로 우리를 이끌어갈 것이기 때문이다. 독재자를 평화적인 방법으로 쫓아낼 수 있는 국민이라면 무슨
							일이든 못하겠는가. 어떤 어려움이든 극복하지 못하겠는가.』</div>
						<hr>
						<h3>이 책이 속한 분야</h3>
						<div class="text" style="border: 1px solid black;">
							<P>사회 > 정치/외교 > 정치일반</P>
					...
					<div id="review" class="tab-pane fade">
						...
						<form>
							...
							<strong>평점:</strong> <input type="number" min="1" max="5"
								placeholder="1~5" style="width: 50px" />점<br>
							<br>
